...
class GeneralController extends Controller
{
    ...
    public function tech_web_banner_title_store(Request $request)
    {
        ...
        Image::make($image)->resize(1920, 466)->save('backend/banner_image/' . $image_gen);
    ...
    public function tec_web_logo_store(Request $request)
    {
        ...
        if ($logo) {
            ...
            if ($request->file('login_bk_img')) {
                @unlink($logo_img->login_bk_img);
                $admin_login_bk_img = $request->file('login_bk_img');
                $admin_login_bk_img_Name = hexdec(uniqid()) . '.' . $admin_login_bk_img->getClientOriginalExtension();
                Image::make($admin_login_bk_img)->resize(1960, 930)->save('backend/loginbgr/' . $admin_login_bk_img_Name);
                $login_bk_img_save = 'backend/loginbgr/' . $admin_login_bk_img_Name; //image save into db
                $data['login_bk_img'] = $login_bk_img_save;
            }
            if ($request->file('admin_logo_image')) {
                ...
                Image::make($admin_logo_img)->resize(300, 86)->save('backend/logo/' . $admin_logo_Name);
            ...
            if ($request->file('frontend_logo_image')) {
                ...
                Image::make($frontend_logo_img)->resize(300, 86)->save('backend/logo/' . $frontend_logo_Name);
            ...
            if ($request->file('frontend_footer_image')) {
                ...
                Image::make($frontend_footer_img)->resize(300, 86)->save('backend/logo/' . $frontend_footer_Name);
            ...
            if ($request->file('favicon_image')) {
                ...
                Image::make($favicon_img)->resize(32, 32)->save('backend/logo/' . $favicon_Name);
            ...
            Logo::insert([
                ...
                'login_bk_img' => $login_bk_img_save,
    ...
    public function tec_web_website_banner_store(Request $request)
    {
        $image = $request->file('banner_image');
        $image_gen = hexdec(uniqid()) . '.' . $image->getClientOriginalExtension();
        Image::make($image)->resize(1920, 920)->save('backend/website_banner/' . $image_gen);
