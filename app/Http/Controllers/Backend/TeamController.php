...
class TeamController extends Controller
{
    ...
    //forntend all methods ============================================

    public function tech_web_team_player(){
        $banner = BannerAndTitle::where('status',1)->where('page','team')->first();
        $players = Team::where('committee','player')->orderBy('id','DESC')->get();
        return view('frontend.team.player_list',compact('players','banner'));
    }  // team status active inactive method end ------------

    public function tech_web_team_coache(){
        $banner = BannerAndTitle::where('status',1)->where('page','team')->first();
        $coache = Team::where('committee','coache')->orderBy('id','DESC')->get();
        return view('frontend.team.coache_list',compact('coache','banner'));
    }  // team status active inactive method end ------------

    public function tech_web_team_judges(){
        $banner = BannerAndTitle::where('status',1)->where('page','team')->first();
        $judge = Team::where('committee','judge')->orderBy('id','DESC')->get();
        return view('frontend.team.judge_list',compact('judge','banner'));
    }  // team status active inactive method end ------------
